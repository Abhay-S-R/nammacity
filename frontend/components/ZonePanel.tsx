...
const getFactorIcon = (icon: string) => {
  switch (icon) {
    ...
    case "cloud-rain": return <CloudRain className="w-3.5 h-3.5" />;
    ...
    case "alert-triangle": return <TriangleAlert className="w-3.5 h-3.5" />;
    default: return <AlertTriangle className="w-3.5 h-3.5" />;
  }
};

export default function ZonePanel({ zone, onClose, onReportIssue }: ZonePanelProps) {
  if (!zone) return null;

  return (
    <div className="absolute top-0 right-0 h-full w-full max-w-md bg-zinc-950/95 backdrop-blur-xl border-l border-zinc-800 z-20 flex flex-col shadow-2xl overflow-hidden">
      {/* Header */}
      <div className="px-6 py-5 border-b border-zinc-800 flex justify-between items-start shrink-0">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-0.5 rounded text-xs font-semibold uppercase tracking-wider bg-zinc-800 text-zinc-300">
              {zone.type.replace(/-/g, " ")}
            </span>
            {zone.scores.severity === "critical" && (
            ...
            {zone.scores.severity === "warning" && (
              <span className="px-2 py-0.5 rounded text-xs font-semibold uppercase tracking-wider bg-yellow-500/20 text-yellow-500 flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
        {/* Description */}
        <p className="text-zinc-400 text-sm mb-6 leading-relaxed">{zone.description}</p>
          ...
          <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex items-center gap-2">
                {zone.scores.trend === "rising" ? (
                  <TrendingUp className="w-4 h-4 text-red-500" />
                ) : zone.scores.trend === "falling" ? (
                  <TrendingDown className="w-4 h-4 text-green-500" />
                ) : (
                  <Minus className="w-4 h-4 text-zinc-500" />
                )}
                <span
                  className={`text-xl font-bold font-mono ${getScoreColor(zone.scores.composite)}`}
                >
            ...
            <div className="w-full bg-zinc-800 rounded-full h-1.5 mb-1 overflow-hidden">
              <div
                className={`h-1.5 rounded-full ${getScoreBgColor(zone.scores.composite)} transition-all duration-500`}
                style={{ width: `${(zone.scores.composite / 10) * 100}%` }}
              ></div>
            </div>
          </div>
        ...
        {/* Factors — now rendering the rich objects from backend */}
        {zone.scores.factors && zone.scores.factors.length > 0 && (
          <div className="mb-8">
            ...
            <ul className="space-y-2">
              {zone.scores.factors.map((factor: ZoneFactor, idx: number) => (
                <li
                  key={idx}
                  className={`flex items-center gap-2 text-sm p-2.5 rounded-lg border ${getImpactColor(factor.impact)}`}
                >
                  {getFactorIcon(factor.icon)}
                  <span>{factor.label}</span>
                  <span className="ml-auto text-[10px] uppercase tracking-wider opacity-70 font-semibold">
                    {factor.impact}
                  </span>
                </li>
              ))}
        ...
        {/* Dispatch Actions */}
        <ActionButtons zone={zone} onReportIssue={onReportIssue} />
      </div>
    </div>
  );
}
...
function ScoreTile({
  ...
  return (
    <div className="p-3 bg-zinc-900/50 rounded-xl border border-zinc-800 flex flex-col items-center justify-center gap-1">
      <div className="text-zinc-400 [&>svg]:w-4 [&>svg]:h-4 mb-1">{icon}</div>
      ...
      <div className="w-full bg-zinc-800 rounded-full h-1 mt-1 overflow-hidden">
        <div
          className={`h-1 rounded-full ${getScoreBgColor(score)} transition-all`}
